  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <main className="max-w-5xl mx-auto p-8">
        {/* Start Here Section without Explore link, centered text */}
        <section className="mb-12 text-center">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Start Here</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {templates.map((tpl, idx) => (
              <div key={idx} className="bg-white rounded-2xl p-6 shadow">
                <h3 className="text-lg font-medium text-gray-800 mb-2 text-center">{tpl.title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed text-center">{tpl.desc}</p>
              </div>
            ))}
          </div>
        </section>

        {/* Growth Goals centered */}
        <section className="text-center">
          <h2 className="text-xl font-semibold text-gray-800 mb-2">Hit Your Growth Goals</h2>
          <p className="text-gray-500 text-sm mx-auto max-w-md">
